...
$dbres = $mysql_cli->query('SELECT * from address where isDistributed=1 and blockchainType=1');
...
for ($row_no = 0; $row_no<$dbres->num_rows; $row_no++ ) {
	...
	$address = $row['address'];
	// get txs for address
	$txs = file_get_contents('https://api.blockcypher.com/v1/eth/main/addrs/'.$address.'?token='.API_TOKEN);
