...
$file = json_decode(file_get_contents('btc_transactions.json'),true);

// read up current counters for brands
foreach ($file as $addr=>$t) {
	// get txs for address
	$txs = file_get_contents('https://api.blockcypher.com/v1/btc/main/addrs/'.$addr.'?token='.API_TOKEN);
	...
	if (array_key_exists('txrefs',$jres)) {
		foreach ($jres['txrefs'] as $tx) {
			...
			// get tx
			$jtxs = json_decode( file_get_contents('https://api.blockcypher.com/v1/btc/main/txs/'.$tx['tx_hash'].'?token='.API_TOKEN),true);
			...
			$txdata['txid'] = $tx['tx_hash'];
			$txdata['confirmations'] = 0;
			$txdata['amount'] = round($tx['value'] / 1e8, 8);

			//open connection
			$ch = curl_init();
	
			//set the url, number of POST vars, POST data
			curl_setopt($ch,CURLOPT_URL,'https://my.qilin.market/Token/Serve/btcIncome');
			...
			//set the url, number of POST vars, POST data
			curl_setopt($ch,CURLOPT_URL,'http://my.fishbank.io/api/btc/callback');
			curl_setopt($ch,CURLOPT_POST,1);
			curl_setopt($ch,CURLOPT_RETURNTRANSFER, true);
			curl_setopt($ch,CURLOPT_FOLLOWLOCATION, true);
			curl_setopt($ch,CURLOPT_POSTFIELDS,$txdata);
